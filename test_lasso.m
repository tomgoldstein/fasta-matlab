...
fprintf('Testing LASSO with N=%d, M=%d\n',N,M);
...
mu = .8*norm(x,1);  %  regularization parameter
...
[sol, outs_adapt] = fasta_lasso(A,A',b,mu,x0, opts);

% Turn on FISTA-type acceleration
opts.accelerate = true;
[sol, outs_accel] = fasta_lasso(A,A',b,mu,x0, opts);

% Do plain old vanilla FBS
opts.accelerate = false;
opts.adaptive = false;
[sol, outs_fbs] = fasta_lasso(A,A',b,mu,x0, opts);
